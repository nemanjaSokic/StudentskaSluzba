...
public class PohadjaUI {
	private static void ispisiMenu() {
		...
		System.out.println("\tOpcija broj 5 - spisak predmeta i studenata koji ih pohadjaju");
		System.out.println("\t\t ...");
	...
	private static void predmetIstudenti() throws SQLException {
		
		List<Predmet> sviPredmeti = new ArrayList<Predmet>();
		List<Student> sviStudentiPredmeta= new ArrayList<Student>();
		
		sviPredmeti = PredmetDAO.getAll(ApplicationUI.conn);
		for (Predmet predmet : sviPredmeti) {
			sviStudentiPredmeta = PohadjaDAO.getPredmetByStudenti(ApplicationUI.conn,predmet);
			System.out.println("Predmet " + predmet.getNaziv());
			for (Student student : sviStudentiPredmeta) {
				System.out.println(student.getIme() + " " + student.getPrezime() + " " + student.getIndeks());
